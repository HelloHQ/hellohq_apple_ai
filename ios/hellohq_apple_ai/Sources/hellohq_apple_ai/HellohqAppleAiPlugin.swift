...
/// iOS plugin exposing Apple's on-device Foundation Model to Flutter:
/// MethodChannel (availability/respond) + EventChannel (streamResponse →
/// cumulative snapshots; the Dart side diffs them). `@available`-guarded (iOS 26).
public class HellohqAppleAiPlugin: NSObject, FlutterPlugin {
    ...
    case "respond":
      ...
      guard #available(iOS 26.0, *) else {
        result(FlutterError(code: "unavailable",
                            message: "The on-device model requires iOS 26 or later.",
                            details: nil))
        return
      }
      Task {
  ...
  static func availabilityName() -> String {
    guard #available(iOS 26.0, *) else { return "unavailableUnknown" }
